...
def build_query_text(req: QuoteRequest) -> str:
    ...
    if req.past_medical_history: parts.append(f"Past medical: {req.past_medical_history}")
    if req.family_medical_history: parts.append(f"Family medical: {req.family_medical_history}")
    ...
    return " | ".join(parts) if parts else "General health insurance quote"